...
public class FechaRecebimentoOnline implements LogicaDeNegocio{

	@SuppressWarnings("unchecked")
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {

		HttpSession sessao = request.getSession();
		String idRecebimentoTempIncluido="";
		int retorno=0;
		...
		if(sessao.getAttribute("listaRecibo")!=null){
			...
			if(retorno>=0){ //---0 = mesmo mês; 1 = mudou mês.
				...
				idRecebimentoTempIncluido=Integer.toString(data.get(Calendar.YEAR))+Integer.toString(numeroRecibo);
				retorno=Integer.parseInt(idRecebimentoTempIncluido);
				...
				objRecibo.setAno(data.get(Calendar.YEAR));
				objRecibo.setNumero(numeroRecibo);
				sessao.setAttribute("objRecibo", objRecibo);
