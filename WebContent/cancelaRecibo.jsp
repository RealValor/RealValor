	  ...
	  <div class="hero-unit" align="center">
		  <div id="estrutura">
	        <table width="764" frame="void">
			   ...
			   <tr><td colspan="3"><div align="left" style="font-family: Arial, Helvetica, sans-serif; font-size: 16px; color: #666666; font-weight: bold;">Cancela Recibo</div></td></tr>     
			   <tr><td colspan="17">&nbsp;</td></tr>	  
		    </table>
			...
			<form id="formNome" name="formNome" method="post">
	        	...
	        	<tr>
				  ...
				  <input class="input-xxlarge" type="text" name="consulta" id="consulta" size="50" maxlength="100" style="font-size:18px; color:#006633; font-weight:bold" value="${objPaga.nome}" onkeyup="pesquisar(this.value,event)" onclick="pesquisa_test_click(this,event);" onblur="clearTime()"/>
	            ...
	            <tr>
	            	<td colspan="4"><div align="left" class="style6">Nº. Recibo<img src="imagens/spacer_1x1px.png" width="2" border="0" />
	            	  <c:set var="posi" value="5"/>
						 ...
						 <input class="input-small" name="numero_recibo" type="text" id="numero_recibo" style="font-size: 16px; color: #006633; font-weight: bold;" size="4" maxlength="6" title="Numero do Recibo" value='<f:formatNumber value="${numeroRecibo}" type="number" pattern="######"/>' onclick='document.getElementById("numero_recibo").value=""' onkeypress="return soNumero(event);" />
						 <img src="imagens/spacer_1x1px.png" width="5" border="0" />Ano<img src="imagens/spacer_1x1px.png" width="5" border="0" />			      
					  </c:if>
	               	  <c:if test="${not empty listRecibo}">
				          <select class="input-medium" name="recibo" id="recibo" style="font-size: 16px; color: #006633; font-weight: bold;" onblur='document.getElementById("numero_recibo").value=recibo.value;' >
					          <c:forEach var="recibos" items="${listRecibo}">
							    <c:if  test="${fn:length (listRecibo) gt 1}">
									<option style='color: #CC3300' value="${recibos.recibo}"><f:formatNumber value='${recibos.recibo}' pattern="0000" />/<f:formatDate value="${recibos.data.time}" type="date" pattern="yyyy" /></option>
							    </c:if> 
							    <c:if  test="${fn:length (listRecibo) == 1}">
									<option value="${recibos.recibo}"><f:formatNumber value='${recibos.recibo}' pattern="0000" />/<f:formatDate value="${recibos.data.time}" type="date" pattern="yyyy" /></option>
							    </c:if>
			        		 </c:forEach>
		                  </select>
					  </c:if>
	               	  <c:if test="${not empty listRecibo}">
					      <input name="numero_recibo" id="numero_recibo" type="hidden" value="${numeroRecibo}" />Ano<img src="imagens/spacer_1x1px.png" width="5" border="0" />                
					  </c:if>
					  <select class="input-small" name="ano" id="ano" style="font-size: 16px; color: #006633; font-weight: bold;" onchange='document.getElementById("ano_paga").value=ano.value'>
	                   ...
	                   <input name="mesini_paga" id="mesini_paga" type="hidden" value="${objMovIni.mes}" title="Escolha o mes inicial na caixa de seleção" size="1" maxlength="1" readonly/>
		               <input type="button" onclick="enviar('ConsultaReciboNumero&numero_recibo='+document.getElementById('numero_recibo').value+'&ctrl=cancelar')" name="bt5" value="Consultar">
	            	</div></td>
	            </tr>
	            </table>
	            <table width="763" frame="void" >
	                	...
	                	<td colspan="3">
	                    	<div align="left"> 
	                    		<c:if test="${objUsu.nivel > 2 }">
		                    		<c:if test="${not empty listaRec && excluido != 'S'}">
							        	<input type="button" onclick="enviar('CancelaRecibo&numero_recibo='+document.getElementById('numero_recibo').value)" name="bt1" value="Cancelar Recibo">
		                    		</c:if>
								</c:if>
		...
		<script>
  	    window.onload = function(){
  	  	    ...
  	  	    if(document.getElementById('cargarecibo').value*1>0){
  	  	    	enviar('ConsultaReciboNumero&numero_recibo='+document.getElementById('numero_recibo').value+'&ano_paga='+document.getElementById('anoRecibo').value+'&ctrl=cancelar');
  	  	    }else{
				if(document.getElementById("cancelado").value=='S'){ 
		    		if(confirm("Visualiza recibo cancelado?")){
						var width = 900;
						var height = 500;
						...
						document.getElementById("cancelado").value='N';
					}
	    		}
  	  	    }
  	    };
		...
		var result;
		  	    ...
		  	    if ((code == 13)||(code == 27)) { //Caso enter passa o foco para o campo de pesquisa, e desativa a listbox. 
				   ...
				   if (code == 27) { //Caso escape (ESC), apaga o valor do campo de pesquisa, 
				      ...
				      //document.getElementById("tipo_entrada").value=0;
					   ...
					   enviar('ConsultaRecibo&controle=cancelar');
				   }
				   document.getElementById("consulta").focus();  		
			    }
		     }
		 ...
		 function preencheCampo(pCampo,pDesc){
     	 ...
     	 function enviar(pModu){
			...
			if(pModu!='CancelaRecibo'&&pModu!='ConsultaRecibo'){
				if(document.getElementById('numero_recibo').value=''){
					if(!(preencheCampo(document.formNome.consulta,'Nome'))){				
						...
						if(recebeCampo!="A"&&recebeCampo!=""){
							if (!confirm(recebeCampo == 'F' ? "Associado Afastado!":recebeCampo == 'L' ? "Associado em Licença!":recebeCampo == 'T' ? "Associado Transferido!":"Nao Associado!")) return false;
						}
					}
				}
			}
			...
			if(pModu=='CancelaRecibo'&&!confirm("Confirma o cancelamento do recibo?")){
				return false;
			}else{
    		    buscaDados(pModu);
			}
		 ...
		 function limpar(){
			    ...
			    buscaDados('ConsultaRecibo&controle=cancelar');
		 }
