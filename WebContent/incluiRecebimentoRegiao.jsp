         	  ...
         	  <td colspan="4">
				  ...
				  <img src="imagens/spacer_1x1px.png" width="1" border="0" />					
	                 <select class="input-medium" name="situacao" id="situacao" style="font-size: 16px; color: #006633; font-weight: bold;" onchange='document.getElementById("situ_paga").value=situacao.value ' >
	                    ...
	                    <option value="${situPaga[0]=='N'?'A':'N'}">${situPaga[0]=='N'?'Núcleo':'Não Associado'}</option>
	                 </select>
            ...
            <table width="760" class="table table-condensed">
		       ...
		       <c:forEach var="linhasrecibo" items="${listaRecibo}">
					...
					<c:forEach var="i" begin="0" end="11">
           	        ...
           	        <td><div align="center"><a href='javascript: confirmaExclusao("IncluiRecebimento&codsocio="+${linhasrecibo.socioDevedor.codigo}+"&codtipo="+${linhasrecibo.entrada.codigo}+"&cod_lista="+${linhasrecibo.numeroLista}+"&ano_paga="+${linhasrecibo.ano})' title="Excluir"><img src="imagens/excluir.png" width="16" height="16" border="0" align="middle" /></a></div></td>
                  ...
                  <td><div align="right" style="font-size: 12px; color: #000000;"><strong><f:formatNumber value="${vlrtotalgeral}" minFractionDigits="2" /></strong></div></td><td>&nbsp;</td></tr>
                  ...
                  <tr>
	               <td colspan="17">
		               <div align="left" class="style6">Observação<img src="imagens/spacer_1x1px.png" width="5" border="0" />         	 
						  <input class="input-xxlarge" type="text" name="observacao" id="observacao" size="70" maxlength="100" style="font-size:18px; color:#006633; font-weight:bold" title="Para inclusão em mês pago, selecione o Tipo antes de digitar o Nome" value="" /></div>
	               </td>
               	  </tr>
                	...
                	<td>
                    	<div align="left">
                    		<c:if test="${not empty listaRecibo}">
						       	...
						       	<c:if test="${objUsu.nivel != 2 }">
						       		<c:if test="${mudouMes == 1 }">
							       		<input type="button" onclick="reservarRecibo()" name="bt5" value="Fechar Recibo">
						       		</c:if>
						       		<c:if test="${mudouMes != 1 }">
								  		<input type="button" onclick="enviar('FechaRecibo')" onfocus="" name="bt2" value="Fechar Recibo">
						       		</c:if>
						       	</c:if>
				...
				if (document.getElementById("controle").value*1==0) {
					...
					if (!confirm("Confirma a finalização do recibo?")) { //imprescindível o posicionamento deste confirm, 
						...
						if (reciboanomes * 1 > 0) {
							...
							var recibo = reciboano.substring(0,
									reciboano.length - 4);
							...
							if (document.getElementById("ano_paga").value * 1 != ano) {
								if (!confirm('O ano ' + document
										.getElementById("ano_paga").value * 1 + ' difere da data do recibo reserva! Fecha assim mesmo?')) {
									return false;
								};
							}
						...
						if (totatual>-1) { //antes totatual == 0 || totatual == 1
							imprimeRecibo();
						} else {
